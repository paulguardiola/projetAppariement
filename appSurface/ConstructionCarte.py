...
def calcul_index(lien , popComp , popRef , url ):
    PP = []
    PN = []
    NP = []
    NN = [] 
    ...
    for i in range(len(lienNonApp)):
        ...
        if inter.area / geomRef.area > 0.7 : 
            ...
            if idLie in lien_1 : 
                ...
                NP.append(popRef[i]['ID'])
            else : 
                NN.append(popRef[i]['ID'])
    ...
    for i in range(len(popRef)):
        ...
        lien["ID"] = idRef
        lien["geometry"] = popRef[i]["geometry"]
        shape.append(lien)
    ...
    for k in range(len(shape)):
        listeID.append(shape[k]["ID"])
        listeGeom.append(shape[k]["geometry"])
        listeIndex.append(shape[k]["index"])
...
if __name__ == '__main__' : 
    ...
    calcul_index(lienFiltres , popComp , popRef , "/Users/guardiola/Desktop/ENSG_troisieme_annee/Alternance/semain_4/testCarte/resultatBD.shp" )
